...
The system SHALL display a clean, structured table for active GTT orders connected to the backend WebSocket receiving the unified GTT payload. Non-active GTTs MUST be filtered out at the source. The table MUST include "Current Price" and "Bought Price" (average price) columns for each GTT order, and match the dense visual styling of the Active Trades dashboard.
...
- **AND** the table rows match the dense height and padding defined for the Active Trades dashboard layout.
